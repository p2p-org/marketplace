...
echo "Create an NFT. Put this NFT on auction. user2 makes a bid greater than opening price. dgaming makes a bid equal to buyout price."
...
echo "put NFT on auction"
echo "mpcli tx marketplace put_on_auction $nft_id 200token $seller_id 5h -u ${boutp}token --from user1"
sleep 2
mpcli tx marketplace put_on_auction $nft_id 200token $seller_id 5h -u ${boutp}token --from user1
...
echo "user2 makes bid"
echo "mpcli tx marketplace bid $nft_id $buyer_id -c 0.045 ${bidp}token --from user2"
sleep 2
mpcli tx marketplace bid $nft_id $buyer_id -c 0.045 ${bidp}token --from user2
...
echo "dgaming makes buyout bid"
echo "mpcli tx marketplace bid $nft_id $buyer_id -c 0.04 ${boutp}token --from dgaming"
sleep 2
mpcli tx marketplace bid $nft_id $buyer_id -c 0.04 ${boutp}token --from dgaming
...
then
      ...
      echo "SUCCESS: $(mpcli query marketplace nft $nft_id)"
fi
