...
sleep_time=5
echo "run test 07.2:"
echo "Create an NFT. Put this NFT on auction. user2 makes a bid equal opening price."
...
mpcli tx marketplace bid $nft_id $buyer_id 195token -c 0.02  --from user2 -y <<< '12345678' >/dev/null
...
last_bid=$(mpcli query marketplace auction_lot $nft_id | grep -oP '(?<=\"last_bid\": )(.*)(?=\,.*)' -m 1 | tr -d ,)

if [[ $status == "on_auction" ]] && [[ $last_bid == "null" ]]
then
      echo "test SUCCESS, nft is on auction, last_bid is null: $(mpcli query marketplace auction_lot $nft_id)"
else
      echo "test FAILURE"
      exit 1
fi