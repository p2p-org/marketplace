...
echo "run test 11:"
echo "Create an NFT. Put this NFT on auction. user2 makes a bid greater than opening price. dgaming performs a buyout."
...
mpcli tx marketplace bid $nft_id $buyer_id -c 0.045 220token --from user2 -y <<< '12345678' >/dev/null
...
mpcli tx marketplace buyout $nft_id $buyer_id -c 0.04 --from dgaming -y <<< '12345678' >/dev/null
