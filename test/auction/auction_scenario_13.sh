...
echo "run test 13:"
echo "Create an NFT. Put this NFT on auction. user2 makes a bid. Finish auction."
...
mpcli tx marketplace bid $nft_id $buyer_id -c 0.04 ${bidp}token --from user2 -y <<< '12345678' >/dev/null
...
mpcli tx marketplace finish_auction $nft_id --from user1 -y <<< '12345678' >/dev/null
...
if [[ $status == "default" ]] && [[ $owner == $user2_id ]] && [[ -z $auc_id ]]
then
      echo "lot went to user2"
else
      echo "test FAILURE"
      exit 1
fi

if [[ $balance_u1 != $(($bidp+(1000-($bidp/20)))) ]] || [[ $balance_u2 != $((1000-$bidp)) ]] || [[ $balance_sb != $((1000+($bidp/50))) ]] || [[ $balance_bb != $((1000+($bidp/50))) ]] || [[ $balance_dg != 1000 ]]
then
